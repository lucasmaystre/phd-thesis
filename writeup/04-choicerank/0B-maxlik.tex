...
From the definition of choice probabilities in~\eqref{cr:eq:wsinglelik}, it is clear that the likelihood is invariant to a rescaling of the parameters, i.e., $\ell(\bm{\gamma}; \mathcal{D}) = \ell(s \bm{\gamma}; \mathcal{D})$ for any $s > 0$.
...
The \emph{comparison graph} induced by $\{ a_{ij} \}$ is the directed graph $\mathcal{H} = (\mathcal{V}, \mathcal{E}')$, where $(i,j) \in \mathcal{E}'$ if and only if there is a node $k$ such that $i, j \in \mathcal{N}^+_k$ and $a_{kj} > 0$.
\end{definition}
...
\begin{theorem}
\label{cr:thm:mlboth}
Let $\mathcal{G} = (\mathcal{V}, \mathcal{E})$ be a directed graph and $\{ (c^-_i, c^+_i) \}$ be the aggregate number of transitions arriving in and originating from $i$, respectively.
Let $\{ a_{ij} \}$ be any set of non-negative real numbers that satisfy
\begin{align*}
\sum_{j \in \mathcal{N}^-_i} a_{ji} = c^-_i, \quad
\sum_{j \in \mathcal{N}^+_i} a_{ij} = c^+_i.
\end{align*}
...
\begin{proof}
The log-likelihood~\eqref{cr:eq:wloglik} is not concave in $\bm{\gamma}$, but it can be made concave using the reparametrization $\gamma_i = e^{\theta_i}$.
...
Partition the vertices into two non-empty subsets $\mathcal{S}$ and $\mathcal{T}$ such that there is no hyperedge between $\mathcal{S}$ to $\mathcal{T}$ in the comparison hypergraph.
Let $\mathcal{A} = \{ i : \mathcal{N}^+_i \subset \mathcal{S} \}$ and $\mathcal{B} = \{ i : \mathcal{N}^+_i \subset \mathcal{T} \}$.
By construction of the comparison hypergraph, $\mathcal{A} \cap \mathcal{B} = \varnothing$ and $\mathcal{A} \cup \mathcal{B} = \mathcal{V}$.
The log-likelihood can therefore be rewritten as
\begin{align*}
\ell(\bm{\theta}) =
    \sum_{i \in \mathcal{A}} \sum_{j \in \mathcal{N}^+_i} a_{ij} \bigg[ \log \gamma_j - \log \sum_{k \in \mathcal{N}^+_i} w_{ik} \gamma_k \bigg]
    + \sum_{i \in \mathcal{B}} \sum_{j \in \mathcal{N}^+_i} a_{ij} \bigg[ \log \gamma_j - \log \sum_{k \in \mathcal{N}^+_i} w_{ik} \gamma_k \bigg].
\end{align*}
...
\begin{align*}
\sum_{j \in \mathcal{N}^-_i} a_{ji} &= c^-_i, \\
\sum_{j \in \mathcal{N}^+_i} a_{ij} &= c^+_i.
\end{align*}
...
In this example, the (data-dependent) comparison graph is \emph{not} strongly connected, and it is easy to see that the likelihood can always be increased by increasing $\gamma_1$, $\gamma_2$ and $\gamma_4$.
