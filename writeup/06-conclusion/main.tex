...
Often, the complexity of the models used to analyze the data (as characterized, e.g., by the number of items) grows concurrently to the amount of data.
...
\begin{description}
\item[Side information] With the exception of Chapter~\ref{ch:playerkern}, we have assumed that the item strengths $\{ \gamma_i \}$ or $\{ \theta_i \}$ are free parameters.
...
Hence, a small fraction of outliers (due, e.g., to the actions of dishonest users) might affect the model significantly.
